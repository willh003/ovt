...
class ImageSaver:
    ...
    def save_depth(self, input_depth_image: Image):
        ...
        try:
            ...
            print("maximum reading=", decoded_depth_image[decoded_depth_image > 0].min(),"(mm)")
            print("minimum >0 reading=", decoded_depth_image.max(), "(mm)")
            ...
            depth_img_vis = cv2.applyColorMap(depth_img_vis, cv2.COLORMAP_TURBO)
            if self.SAVE_DEPTH:
