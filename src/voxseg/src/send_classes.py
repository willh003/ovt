...
def send_classes(client: VoxSegClient, classes=None):

    if type(classes) == list:
        client.publish_class_names(classes, None, False)
    elif type(classes) == dict:
        client.publish_class_names(None, classes, True)
...
def main():
    ...
    # NOTE: THESE LINES ARE NECESSARY 
    ...
    class_names = ['ground', 'rock', 'brick', 'fire hydrant', 'heavy machinery']
    send_classes(client, class_names)
