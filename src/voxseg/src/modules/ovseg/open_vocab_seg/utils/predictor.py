...
class OVSegPredictor(DefaultPredictor):
    ...
    def batch_inference(self, images, class_names):
        """
        Will thing
        images should be in rgb format, shape (H,W,C)
        """
        with torch.no_grad():  # https://github.com/sphinx-doc/sphinx/issues/4258
            # Apply pre-processing to image.
            inputs = []
            ...
            for i in range(len(images)):
                original_image = images[i][:, :, ::-1]
                image = self.aug.get_transform(original_image).apply_image(original_image)
                image = torch.as_tensor(image.astype("float32").transpose(2, 0, 1))
                input = {"image": image, "height": height, "width": width, "class_names": class_names}
                inputs.append(input)
